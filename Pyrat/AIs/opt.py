...
def threeOpt(mazeMap, piecesOfCheese, playerLocation):
    ...
    for i in range(1,len(orderCheese)):
        for j in range(i + 1, len(orderCheese)):
            for k in range(j + 2, len(orderCheese)):
                deltaW, o = swapIfBetter(orderCheese, i, j, k)
...
def swapIfBetter(orderCheese, i, j, k):
    ...
    if d0 > d1:
        ...
        pivot.reverse()
        return (d1-d0, orderCheese[:i] + pivot + orderCheese[j:])
    elif d0 > d2:
        pivot = orderCheese[j:k]
        pivot.reverse()
        return (d2-d0, orderCheese[:j] + pivot + orderCheese[k:])
    elif d0 > d3:
        pivot = orderCheese[j:k] + orderCheese[i:j]
        pivot.reverse()
        return (d3-d0, orderCheese[:i] + pivot + orderCheese[k:])
    elif d0 > d4:
        pivot = orderCheese[i:k]
        pivot.reverse()
        return (d4-d0, orderCheese[:i] + pivot + orderCheese[k:])
    else:
